            ...
            ContentManager.ResCallback callbackOuter = (callbackRes) =>
            {
                ...
                ContentManager.ResCallback callbackFormat = (result) =>
                {
                  ...
                  //   {
                        ...
                        MOST LIKELY THE SCRACTH BUFFER GETS RE-USED AND OVERWRITTEN
                             levelDataSizeInBytes[level] = reader.ReadInt32 ();
                             levelData[level] = reader.ContentManager.GetScratchBuffer (levelDataSizeInBytes[level]);
                             reader.Read (levelData[level], 0, levelDataSizeInBytes[level]);

                         }

                         // start task that puts the data above into textures
                         ContentManager.EnqueueResourceLoadingTask (new ContentManager.ResTask (
                               (res) =>
                              {
                                           ...
                                           switch (surfaceFormat)
                                           {
                                               ...
                                               case SurfaceFormat.Dxt1a:
                                                   if (!reader.GraphicsDevice.GraphicsCapabilities.SupportsDxt1 && convertedFormat == SurfaceFormat.Color)
                                                   {
                                                       ...
                                                       levelDataSizeInBytes[level] = levelData.Length;
                                                   }
                                                   break;
                                               case SurfaceFormat.Dxt3:
                                               case SurfaceFormat.Dxt3SRgb:
