...
namespace Microsoft.Xna.Framework.Content
{
    public class Texture2DReader : ContentTypeReader<Texture2D>
    {
        ...
        protected internal override void ReadCallback(ContentReader reader, Texture2D existingInstance, ContentManager.ResCallback onDone)
        {
            Texture2D texture = null;
...
#if OPENGL
            // Threading.BlockOnUIThread (() =>
            ContentManager.ResCallback callbackOuter = (callbackRes) =>
            {
#endif

#if ROPO_TIME
                System.Diagnostics.Stopwatch swTex = new System.Diagnostics.Stopwatch ();
                swTex.Start ();
#endif
                texture = existingInstance ?? new Texture2D(reader.GraphicsDevice, width, height, levelCountOutput > 1, convertedFormat);
...
#if ROPO_TIME
                swTex.Stop ();
                ContentManager.addTime("Texture2D::ReadCallback new Texture2D",swTex.ElapsedMilliseconds);
#endif
                //  ContentManager.ResCallback callbackFormat = (result) =>
                //  {
#if ROPO_TIME
                    System.Diagnostics.Stopwatch swFormat = new System.Diagnostics.Stopwatch ();
                    swFormat.Start ();
#endif
                // TODO: 4+ THREADS LOADING AT SAME TIME CAN CAUSE A HUGE MEMORY SPIKE BECAUSE HOLDING ALL DATA IN HERE
                ...
                for (int level = 0; level < levelCount; level++)
                {
                    // internalSw.Start ();
                    byte[] levelData = null;
                    int levelDataSizeInBytes = 0;

                    levelDataSizeInBytes = reader.ReadInt32();
                    // SCRATCH BUFFER CANNOT BE REUSER ACROSS LOOPS SINCE IT REUSES DATA!
                    // JUST TO LAZY TO REMOVE the array structures
                    levelData = reader.ContentManager.GetScratchBuffer(levelDataSizeInBytes);
                    reader.Read(levelData, 0, levelDataSizeInBytes);
...
#if ROPO_TIME
                    swFormat.Stop ();
                    ContentManager.addTime ("Texture2D::ReadCallback Format", swFormat.ElapsedMilliseconds);
#endif
                onDone(texture);
            ...
            //   ContentManager.EnqueueResourceLoadingTask (new ContentManager.ResTask (callbackFormat));
            // };
#endif

            ContentManager.EnqueueResourceLoadingTask(new ContentManager.ResTask(callbackOuter));
        }
